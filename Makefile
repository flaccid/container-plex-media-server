...
helm-install:: ## installs using helm from chart in repo
		@helm install --name plex-media-server ./charts/plex-media-server
...
helm-purge:: ## deletes and purges deployed helm release
		@helm delete --purge plex-media-server

helm-render:: ## prints out the rendered chart
		@helm install --dry-run --debug charts/plex-media-server
